            ...
            if (self.config.addComputerSMB is not None):
                ...
                try:
                    if self.config.command is not None:
                        ...
                        # Disables remote UAC protections to allow for execution via nxc smb
                        LOG.info("Attempting to disable remote UAC protections on host: %s", self.__SMBConnection.getRemoteHost())
                        remoteOps.disableProtections()
                        LOG.info("Done disabling remote UAC protections")
                except Exception as e:
                    LOG.error(str(e))
                finally:
